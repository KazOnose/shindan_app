...
SYSTEM_PROMPT_DIAGNOSIS_WITH_PATTERN = """
    ...
    2. 診断結果は次の4つの内容を必ず埋めてください。
       ...
       - 御社の場合: 200〜300字で、次の順に書く。(1) 御社の規模・業種なら普通すでに持っているもの（文脈の資料が「材料」として挙げているもの）を1〜2つ具体名で、「〜があれば」と仮定形で示す。「社内にある材料」が選ばれていればその中のどれから始めるかを優先し、業種の資料（種別: 業種）が文脈にあれば、その資料の「すでにある材料」のうち相談領域（人事／集客・営業／顧客対応／業務効率化）に対応する欄のものを優先する。材料に口頭のものが選ばれた場合は、話を録音して文字にするところから書く。(2) その材料からAIで何を作る・直すかを、文脈の資料にある取り組みの型を1つ引いてこの会社に当てはめ、「〜を作れます」「〜から始められます」の可能形で具体に書く。(3) それで日々の仕事がどう変わりそうかを1文、可能性の表現で書く。(4) 始める前に気をつける点を1文、文脈の資料にあるものだけ書く。御社の現状を説明する文（「〜の状況です」「〜に支障をきたしています」「〜が求められています」）は書かず、困りごとは入力で分かっているので言い換えない。「〜の効率化があれば」のように目的や結果を「〜があれば」に入れない。御社について分かっているのは、業種・従業員数・選んだ困りごと・選んだ材料・補足だけで、それ以外の状態を推測して断定しない。文書がある前提で「最近の〜を用意する」とは書かない
