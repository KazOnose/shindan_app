...
SYSTEM_PROMPT_DIAGNOSIS_WITH_PATTERN = """
    ...
    2. 診断結果は次の4つの内容を必ず埋めてください。
       ...
       - 御社の場合: 2〜3文で書く。御社の現状を説明する文（「〜の状況です」「〜に支障をきたしています」「〜が求められています」）は書かず、困りごとは入力で分かっているので言い換えない。1文目から「過去の問い合わせ履歴があれば、FAQの整備から始められます」のように、材料名と最初の一手を書く（「〜の効率化があれば」のように目的や結果を「〜があれば」に入れない）。この困りごとに対して、御社の規模・業種なら普通すでに持っているもの（文脈の資料が「材料」として挙げているもの）を1〜2つ具体名で示し、材料は「〜があれば」と仮定形で書いてそこから作る・直す内容は「〜を作れます」「〜から始められます」の可能形で書き、「社内にある材料」が選ばれていればその中のどれから始めるかを優先して書く。御社について分かっているのは、業種・従業員数・選んだ困りごと・選んだ材料・補足だけで、それ以外の状態を推測して断定しない。文脈の資料にある取り組みの型を1つ引いてこの会社に当てはめた形で書き、業種の資料（種別: 業種）が文脈にあれば、その資料の「すでにある材料」のうち相談領域（人事／集客・営業／顧客対応／業務効率化）に対応する欄のものを優先して、その業種に合う型を使う。文書がある前提で「最近の〜を用意する」とは書かない。材料に口頭のものが選ばれた場合は、話を録音して文字にするところから書く。
       - 根拠となる数字: 以下の文脈にある調査数値を1〜2個、出典名を添えて書く
       - 次の一歩: 今週できることを1つ、2〜3文で書く
...
SYSTEM_PROMPT_DIAGNOSIS_NO_PATTERN = """
    ...
    3. 診断結果は次の4つの内容を必ず埋めてください。
       ...
       - 御社の場合: 2〜3文で書く。御社の現状を説明する文（「〜の状況です」「〜に支障をきたしています」「〜が求められています」）は書かず、困りごとは入力で分かっているので言い換えない。1文目から「過去の問い合わせ履歴があれば、FAQの整備から始められます」のように、材料名と最初の一手を書く（「〜の効率化があれば」のように目的や結果を「〜があれば」に入れない）。この困りごとに対して、御社の規模・業種なら普通すでに持っているもの（文脈の資料が「材料」として挙げているもの）を1〜2つ具体名で示し、材料は「〜があれば」と仮定形で書いてそこから作る・直す内容は「〜を作れます」「〜から始められます」の可能形で書き、「社内にある材料」が選ばれていればその中のどれから始めるかを優先して書く。御社について分かっているのは、業種・従業員数・選んだ困りごと・選んだ材料・補足だけで、それ以外の状態を推測して断定しない。文脈の資料にある取り組みの型を1つ引いてこの会社に当てはめた形で書き、業種の資料（種別: 業種）が文脈にあれば、その資料の「すでにある材料」のうち相談領域（人事／集客・営業／顧客対応／業務効率化）に対応する欄のものを優先して、その業種に合う型を使う。文書がある前提で「最近の〜を用意する」とは書かない。
...
PARSER_DESC_FOR_YOUR_COMPANY = "入力された材料の名前を挙げて、その材料から具体的に何が作れるかを2〜3文で言い切った文章"
PARSER_DESC_EVIDENCE = "根拠となる調査数値と出典名。数値が無い場合はその旨"
PARSER_DESC_NEXT_STEP = "今週できること1つ（相談への案内は含めない）"
