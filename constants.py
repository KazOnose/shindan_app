...
SYSTEM_PROMPT_DIAGNOSIS_WITH_PATTERN = """
    ...
    2. 診断結果は次の4つの内容を必ず埋めてください。
       ...
       - 御社の場合: 2〜3文で書く。この困りごとに対して、御社の規模・業種なら普通すでに持っているもの（文脈の資料が「材料」として挙げているもの）を1〜2つ具体名で示し、そこから何を作る・直すかを言い切る。「社内にある材料」が選ばれていれば、その中のどれから始めるかを優先して書く。文脈の資料にある取り組みの型を1つ引いて、この会社に当てはめた形で書く。文書がある前提で「最近の〜を用意する」とは書かない。材料が未選択のときは「〜があれば」「〜がある会社なら」と仮定形で書き、あると断定しない。材料に口頭のものが選ばれた場合は、話を録音して文字にするところから書く。業種の資料（種別: 業種）が文脈にあれば、その資料の「すでにある材料」のうち相談領域（人事／集客・営業／顧客対応／業務効率化）に対応する欄のものを優先し、その業種に合う型を使う。
...
SYSTEM_PROMPT_DIAGNOSIS_NO_PATTERN = """
    ...
    3. 診断結果は次の4つの内容を必ず埋めてください。
       ...
       - 御社の場合: 2〜3文で書く。この困りごとに対して、御社の規模・業種なら普通すでに持っているもの（文脈の資料が「材料」として挙げているもの）を1〜2つ具体名で示し、そこから何を作る・直すかを言い切る。「社内にある材料」が選ばれていれば、その中のどれから始めるかを優先して書く。文脈の資料にある取り組みの型を1つ引いて、この会社に当てはめた形で書く。文書がある前提で「最近の〜を用意する」とは書かない。材料が未選択のときは「〜があれば」「〜がある会社なら」と仮定形で書き、あると断定しない。業種の資料（種別: 業種）が文脈にあれば、その資料の「すでにある材料」のうち相談領域（人事／集客・営業／顧客対応／業務効率化）に対応する欄のものを優先し、その業種に合う型を使う。
